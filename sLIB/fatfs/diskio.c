...
DRESULT disk_read(
	...
	HAL_SD_ReadBlocks(&hsd, (uint8_t*)buff, sector, count, 2000);
...
DRESULT disk_write(
    ...
    // 先擦除
    HAL_SD_Erase(&hsd, sector, sector + count);

    while(HAL_SD_GetCardState(&hsd) != HAL_SD_CARD_TRANSFER) {
        // log_printfln("等待SD卡擦除完成...");
    }

    HAL_StatusTypeDef status = HAL_SD_WriteBlocks(&hsd, (uint8_t*)buff, sector, count, 2000);
    if(status != HAL_OK) {
